...
def search_google_books():
    ...
    if request.method == 'POST':
        ...
        response = requests.get(
            f"https://www.googleapis.com/books/v1/volumes?q={query}&key={os.environ.get('API_KEY')}&maxResults=1")
        data = response.json()
        print(data)
        if "items" in data:
